...
Decomposes the active psychometric battery into explicit clinical subdomains:
  - Memory: Pattern Recall, Delayed Word Recall
  - Attention: Digit Span Forward, Reaction Time
  - Working Memory: Digit Span Backward, Sequential Span
  - Visuospatial: Pattern Visual Recall
  - Language: Voice Journal Lexical / Semantic Telemetry
  - Executive Function: Digital Trail Making (Parts A & B), Stroop Interference
  - Processing Speed: Reaction Latency, Trail Making Part A

Calculates % baseline deviations for each subdomain and formulates explainable
non-diagnostic clinical reasoning over multi-test patterns.
"""
...
class CognitiveTestAgent:
    ...
    def analyze(self, test_results: List[Any], baseline: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for t in test_results:
            ...
            # Domain Mapping
            # 1. Executive Function (Trail Making Part B, Stroop)
        ...
        wm_val = round(sum(working_memory_scores) / len(working_memory_scores), 1) if working_memory_scores else 78.0
        ...
        wm_delta_pct = round(((wm_val - base_wm) / max(base_wm, 1.0)) * 100.0, 1)
        visuo_delta_pct = round(((visuo_val - base_visuo) / max(base_visuo, 1.0)) * 100.0, 1)
        exec_delta_pct = round(((exec_val - base_exec) / max(base_exec, 1.0)) * 100.0, 1)
        ...
        # Supports both full 6-domain battery and legacy 5-test battery with mathematical consistency
        has_trail = any("trail" in (t.test_type if hasattr(t, "test_type") else t.get("test_type", "")).lower() for t in test_results)
        ...
        if has_trail:
            ...
            # Preserved exact legacy formula
            cognitive_score = round(
                0.35 * mem_val +
                0.20 * str_val +
                0.15 * rxn_val +
        ...
        wm_status = "declining" if wm_delta_pct < -12.0 else "improving" if wm_delta_pct > 8.0 else "stable"
        visuo_status = "declining" if visuo_delta_pct < -12.0 else "improving" if visuo_delta_pct > 8.0 else "stable"
        exec_status = "declining" if exec_delta_pct < -12.0 else "improving" if exec_delta_pct > 8.0 else "stable"
        ...
        confidence = round(min(0.96, 0.70 + 0.05 * min(len(test_results), 6)), 2)
        ...
        if mem_status == "declining" and exec_status == "declining":
            reasoning = (
                ...
                f"while attention remains relatively preserved. The overall cognitive score ({cognitive_score}/100) "
                f"reflects multi-domain task latency and sequencing shifts."
            )
        elif exec_status == "declining":
            reasoning = (
                f"Executive-function performance has slowed relative to personal baseline "
                f"({subdomain_metrics['executive_function']['interpretation']}), while memory ({mem_val}/100) and "
                f"visuospatial fidelity ({visuo_val}/100) remain aligned with normative expectations."
            )
        elif mem_status == "declining" and spd_status == "declining":
            reasoning = (
            ...
            reasoning = (
                ...
                f"with subtle shifts in {', '.join([k.replace('_', ' ') for k, v in subdomain_metrics.items() if v['status'] == 'declining' and k in ['memory', 'executive_function', 'working_memory', 'attention', 'visuospatial', 'processing_speed']])} "
                f"relative to personal baseline norms."
            )
        else:
            reasoning = (
                f"Multi-domain cognitive battery performance remains stable across Memory ({mem_val}), "
                f"Executive Function ({exec_val}), Working Memory ({wm_val}), and Processing Speed ({spd_val})."
            )
